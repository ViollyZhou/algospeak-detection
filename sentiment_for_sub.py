...
if torch.cuda.is_available():
    ...
    print("Error: No GPU detected. Please enable the T4 x2 accelerator in the settings on the right.")
...
# Kaggle's output file path is /kaggle/working/
...
SENTIMENT_THRESHOLD = 0.5  # Absolute sentiment score threshold

# --- Model Selection ---
# Using an English model
MODEL_PATH = "cardiffnlp/twitter-roberta-base-sentiment-latest"


class DataParallelWithConfig(DataParallel):
    """
    A custom DataParallel wrapper that forwards missing attribute requests
...
def sentiment_filter_multi_gpu(df: pd.DataFrame) -> pd.DataFrame:
    """
    Uses multi-GPU parallel processing to compute sentiment scores and filters data based on a threshold (final revised version).
    """
    ...
    # --- Core Fix: Replace 'DataParallel' with our custom 'smart' wrapper ---
    ...
    # The batch_size here can be adjusted according to your VRAM, e.g., 256 or 512 for 32GB
    batch_size = 256
    ...
    print(f"Data size after filtering: {len(df_filtered)} rows (removed {original_count - len(df_filtered)} rows)")
...
try:
    ...
    print(f"Current data size: {len(data_frame)} rows")
    ...
    print(f"Final remaining data size: {len(final_df)} rows")
    print(f"Final file has been saved to: {OUTPUT_FILE}")
    print("You can find it in the 'Output' directory in the 'Data' panel on the right.")

except FileNotFoundError:
    print(f"Error: File not found '{INPUT_FILE}'.")
    print("Please check if 'YOUR_DATASET_FOLDER_NAME' has been modified correctly.")
